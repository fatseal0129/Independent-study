...
class Mode_out(threading.Thread):
    def __init__(self, p, m):
        super(Mode_out, self).__init__()
        self.predictQueue = p
        self.mainframe = m
        self.daemon = True  # Allow main to exit even if still running.
        self.paused = True  # Start out paused.
    ...
    def run(self):
        while True:
            ...
            # Do stuff...
            frame = self.mainframe.get()
            self.predictQueue.put(dm.forOutDoor(frame))
...
async def read_webscoket(websocket: WebSocket):
    ...
    out_mode = Mode_out(predicted, main_frames)
    out_mode.start()
