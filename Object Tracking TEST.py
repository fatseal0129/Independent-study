...
while True:
    ...
    for objects in abandoned_objects:
        ...
        # _, x2, y2, w2, h2 = objects
        # cv2.putText(predict, "SUS", (x2, y2 - 10), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 0, 255), 2)
        # cv2.rectangle(predict, (x2, y2), (x2 + w2, y2 + h2), (0, 0, 255), 2)



    # cv2.imshow("Image", predict)
    # current_time = time.time()
    # fps = math.ceil(1/(current_time - previous_time))
    # previous_time = current_time
    # count += 1
    # if count == 1:
    #     avgfps = fps
    # else:
    #     avgfps = math.ceil(((avgfps * count + fps) / (count + 1)))
    # cv2.putText(img, f'FPS: {str(avgfps)}', (30, 50), cv2.FONT_HERSHEY_COMPLEX, 1.2, (0, 0, 0), lineType=cv2.LINE_AA)
